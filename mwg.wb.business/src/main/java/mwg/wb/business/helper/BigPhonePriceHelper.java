...
public class BigPhonePriceHelper extends APIPriceHelper {

//	private static final List<Integer> ngungkinhdoanh = Arrays.asList(0, 5, 6);
	private static final Set<Integer> hangmauStatus = Set.of(1, 2);
	...
	public ProductErpPriceBO ProcessProductStatus(ProductErpPriceBO objPrice) {
		if (objPrice != null) {
			if (objPrice.campreorder)
				objPrice.WebStatusId = 6;
			else if (objPrice.IsProductComingForBigphone)
				objPrice.WebStatusId = 2;
			else if (!objPrice.IsWebShow || objPrice.Price == 0 /* || ngungkinhdoanh.contains(objPrice.StatusId) */)
				objPrice.WebStatusId = 1;
			else {
				if (objPrice.ProductCodeTotalQuantity > 0
						|| (trungBay.contains(objPrice.CategoryId) && objPrice.TotalSampleQuantity > 0))
					objPrice.WebStatusId = 4;
				else if (!hangmauStatus.contains(objPrice.StatusId))
