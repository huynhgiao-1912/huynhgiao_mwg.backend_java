...
public class RedisPoolConnection {
    ...
    public static void initializeSettings(String host) {
        RedisPoolConnection.host = host;
        RedisPoolConnection.port = 6379;
        //RedisPoolConnection.password = password;
        RedisPoolConnection.connectTimeout = 10000; //10s // connectTimeout;
        RedisPoolConnection.operationTimeout = 10000; //10s //operationTimeout;
    }
        ...
        if (pool == null) { // avoid synchronization lock if initialization has already happened
            synchronized(staticLock) {
                if (pool == null) { // don't re-initialize if another thread beat us to it.
                    ...
                    int db = 0;
                    String clientName = null; // null means use default
    ...
    public Jedis getResource(int select) {
        Jedis jedis;
        try{
...
//            }
    ...
    public static JedisPoolConfig getPoolConfig() {
        if (config == null) {
            ...
            RedisPoolConnection.config = poolConfig;
        }
