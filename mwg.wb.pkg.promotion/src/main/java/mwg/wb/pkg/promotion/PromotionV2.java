...
public class PromotionV2 implements Ididx {
...
//			}
			...
			if (strNOTE.contains("DIDX_TOP")) {
				 ...
				 notifyHelperLog.NotifyInfo("SERVICE-PROMOTIONV2:" + message.Identify, DataCenter);
			}
...
//			String arecordid11 = siteid + "_11_" + "_-1_" + OutputTypeID + "_" + InventoryStatusID;
			...
			Logs.Log(isLog, strNOTE, "GetPromotionByPrdID V2" + arecordid);
...
//			mwg.wb.model.promotion.Promotion[] allList = new mwg.wb.model.promotion.Promotion[] {};
			...
			try {
				lstpromotion = erpPromotionHelper.GetPromotionByPrdID(productCode, ProvinceID, OutputTypeID, 1,
						SalePrice, BrandID);
...
//				}
				if (listTempPromotion != null) {
					...
					Utils.BuildSql(isLog, strNOTE, "productpromotion", "recordid", recordidcommonV2, item1, ",data,",
							ref1);
				...
				var bklist = listTempPromotion.stream().filter(x -> x.GroupID.equalsIgnoreCase("bankem"))
						.toArray(Promotion[]::new);
				if (bklist != null) {
					for (var bk : bklist) {
